...
import org.yixun.platform.workflow.graph.ActivitiHistoryGraphBuilder;
import org.yixun.platform.workflow.graph.Edge;
import org.yixun.platform.workflow.graph.Graph;
import org.yixun.platform.workflow.graph.Node;
import org.yixun.platform.workflow.spring.ApplicationContextHelper;
...
public class WithdrawTaskCmd implements Command<Integer> {
    ...
    public void collectNodes(Node node, List<String> historyNodeIds) {
        logger.info("node : {}, {}, {}", new Object[]{node.getId(), node.getType(),
                node.getName()});
